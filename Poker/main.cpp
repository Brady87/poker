...
void main() {
	...
	do {//Nouvelle partie
		...
		} while (!(jeu.get_tour() >= 5 || vous.get_choix() == 4 || adversaire.get_choix() == 4 ||(vous.get_jetons()==adversaire.get_jetons() && ((vous.get_jetons() <= 0 && adversaire.get_choix() != 0) || (adversaire.get_jetons() == 0 && vous.get_choix()!=0))))); //Tant que personne n'est couché, a sec, et qu'il reste des tours
		...
		if (rep == 1) { // Seul le serveur effectue les modifs et la sauvegarde
			jeu.set_pot(0); // Pot remis à zéro
